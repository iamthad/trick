...
int convert_united_value( std::string & to_units , std::string & from_units , long long * val ) {
    if ( from_units.compare("1") ) {
        ...
        if ( !from ) {
            PyErr_SetString(PyExc_AttributeError,(std::string("could not convert from units "+from_units).c_str()));
            return -1 ;
        }
        ut_unit * to = ut_parse(Trick::UdUnits::get_u_system(), to_units.c_str(), UT_ASCII) ;
        if ( !to ) {
            PyErr_SetString(PyExc_AttributeError,(std::string("could not convert to units "+to_units).c_str()));
            return -1 ;
        }
